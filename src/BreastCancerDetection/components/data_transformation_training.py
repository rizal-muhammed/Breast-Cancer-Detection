...
from BreastCancerDetection.entity import DataTransformationTrainingConfig
...
class DataTransformationTraining:
    ...
    def delete_existing_good_data_training_folder(self, ):
        ...
        try:
            ...
            common.remove_directories([self.config.good_raw])
    ...
    def delete_exising_bad_data_training_folder(self, ):
        ...
        try:
            ...
            if os.path.isdir(self.config.bad_raw):
                common.remove_directories([self.config.bad_raw])
    ...
    def move_bad_files_to_archive_bad(self, ):
        ...
        try:
            ...
            if os.path.isdir(self.config.bad_raw):
                if not os.path.isdir(self.config.archive_bad_raw):
                    common.create_directories([self.config.archive_bad_raw])
                
                bad_data_raw = "bad_data_" + str(date) + "_" + str(time)
                destination = os.path.join(self.config.archive_bad_raw, bad_data_raw)
                if not os.path.isdir(destination):
                    common.create_directories([destination])
                
                files = os.listdir(self.config.bad_raw)
                for file in files:  # moving bad files to bad arvhive directory
                    if file not in os.listdir(destination):
                        shutil.move(os.path.join(self.config.bad_raw, file), destination)
